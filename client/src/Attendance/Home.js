...
const Home = () => {
    ...
    return (
        <div style={{display:"flex"}}>
           <div className='bg-dark' style={{height:"880px",width:"13%",paddingLeft:"10px"}}>
           <div>
           <Link to={`/teacher/dashboard`} className="link_class" style={{cursor:"pointer"}}> <i className="fas fa-exclamation-square"></i> Profile</Link>
           <hr style={{color:"black"}} />
            </div>
           <div>
                <p  onClick={() => setCshow(!cshow)} style={{fontSize:"20px",color:"white",cursor:"pointer"}}> 
                <i className="fas fa-user-friends" /> Mark Attendance</p>
                {cshow && (
                    <ul style={{listStyleType:"none" }}>
                    {classNum.map(item =>(
                        <>
                      <li key={item} onClick={()=> handleClick(item)} style={{color:"white",fontSize:"20px",height:"30px",textDecoration:"none",cursor:"pointer"}}>
            ...
            <div>
                <p  onClick={()=> setMshow(!mshow)} style={{fontSize:"20px",color:"white",cursor:"pointer"}}>  <i className="fas fa-user-crown"></i>Upload Mark</p>
                {mshow && (
            ...
            <div>
                <p style={{fontSize:"20px",color:"white",cursor:"pointer"}}> <i className="fas fa-exclamation-square"  onClick={() => dispatch(logoutUser())}></i> Logout</p>
            </div>
           </div>
           <div style={{flex:1}}>
                {/* <ul>
