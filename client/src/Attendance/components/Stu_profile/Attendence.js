...
const Attendance = () => {
  ...
  const funData = () => {
    let pt = 0,
      ab = 0;
    currentUser &&
      currentUser[0].attdenList &&
      currentUser[0].attdenList.map((item) => {
    ...
    <div>
      <div style={{ marginTop: "20px" }}>
        ...
        >
          ...
          {currentUser &&
            currentUser[0].attdenList &&
            currentUser[0].attdenList.map((user) => (
              <tbody key={user.name} style={{ padding: "5px" }}>
                <tr
                  className={
                    setclasssfun(user.type) ? "table-danger" : "table-success"
                  }
                >
                  <td>
                    {moment.unix(user.timestamp / 1000).format("DD MMM YYYY")}
                  </td>
